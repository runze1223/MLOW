...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.model_type == 'linear':
            self.model = nn.Linear(self.seq_len*11, self.pred_len)
        elif self.model_type == 'mlp':
            self.model = nn.Sequential(
                nn.Linear(self.seq_len*11, self.d_model),
                nn.ReLU(),
    ...
    def forward(self, x, y,z, cycle_index):
        ...
        x[:,-1,:,:] = x[:,-1,:,:] - self.cycleQueue(cycle_index, self.seq_len)
        ...
        L=self.pred_len
        if self.std:
            y = y * (stdev[:, 0, :].unsqueeze(1).repeat(1, L, 1))


        y = y + (means[:, 0, :].unsqueeze(1).repeat(1, L, 1))
