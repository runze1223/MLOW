...
class expert(nn.Module):
    ...
    def __init__(self, configs, individual=False):
        ...
        self.seq_len = configs.cut1*(configs.seq_len//2)
    ...
    def encoder(self, x):
        ...
        trend_output = self.Linear_Trend(x.squeeze(-1))
        x =  trend_output.unsqueeze(-1)
...
class encoder(nn.Module):
    def __init__(self, config):
        ...
        input_size = config.cut1*(config.seq_len//2)
...
class Linear_extractor_cluster(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.input_size =config.cut1*(config.seq_len//2)
        self.k = config.k
        # instantiate experts
